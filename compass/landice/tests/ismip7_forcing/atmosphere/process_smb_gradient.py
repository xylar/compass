...
class ProcessSmbGradient(Step):
    """
    A step for processing ISMIP7 SMB elevation gradient (dacabfdz) data.
    Remaps the annual SMB gradient from the ISMIP7 2km polar stereographic
    grid to the MALI unstructured mesh. This field is used for
    ice-elevation feedback corrections.
    """
    ...
    def run(self):
        ...
        if not all_files:
            raise FileNotFoundError(
                f"No SMB gradient files found matching pattern:\n"
        ...
        if not input_files:
            raise FileNotFoundError(
                f"No SMB gradient files for year range "
